...
// Notes to Grader: Eric implemented DFS
...
public class MajorGraph<T> implements GraphADT<T> {
    ...
    protected class Edge implements Comparable<Edge> {
        ...
        /**
         * Compares two edges to determine which has the higher weight.
         * 
         * @param other - edge to be compared with
         * @return +1 if this edge's weight is greater than other edge's weight, 0 if
         *         weights are equal, -1 if this edge's weight is less than other edge's
         *         weight
         */
        @Override
        public int compareTo(Edge other) {
            int comparedWeight = this.weight - other.weight;

            if (comparedWeight == 0)
                return comparedWeight;
            else if (comparedWeight > 0)
                return 1;
            else
                return -1;
        }
    ...
    /**
     * Creates a minimum spanning tree starting from the specified vertex using
     * Prim's MST algorithm.
     * 
     * @param startVertex
     * @return a minimum spanning tree of the graph starting from the specified node
     */
    public MajorGraph<T> primsSpanningTree(T courseNode) {
        if (courseNode == null || !vertices.containsKey(courseNode)) {
            throw new NoSuchElementException("Specified vertex does not exist within graph");
        }

        Vertex startVertex = vertices.get(courseNode);

        Set<Vertex> visited = new HashSet<>();
        visited.add(startVertex);

        PriorityQueue<Edge> edgesToProcess = new PriorityQueue<>();
        for (Edge edge : startVertex.edgesLeaving) {
            edgesToProcess.add(edge);
        }

        MajorGraph<T> spanningTree = new MajorGraph<>();
        spanningTree.insertVertex(startVertex.data);

        while (visited.size() < vertices.size()) {
            if (edgesToProcess.isEmpty())
                break;

            Edge edgeToAdd = edgesToProcess.remove();

            if (!visited.contains(edgeToAdd.target)) {
                spanningTree.insertVertex(edgeToAdd.target.data);
                spanningTree.insertEdge(edgeToAdd.source.data, edgeToAdd.target.data, edgeToAdd.weight);
                visited.add(edgeToAdd.target);
                for (Edge edge : edgeToAdd.target.edgesLeaving) {
                    edgesToProcess.add(edge);
                }
            }
        }
    ...
    /**
     * depthFirstSearchHelper. Recursive helper method to the DepthFirstSearch. Will
     * stop when there are no more nodes in the depth to visit.
     * 
     * @param v       Vertex - vertex to start at with current recursive iteration.
     * @param visited LinkedList<Vertex> - list of visited vertices so to prevent
     *                duplicates.
     */
    private void depthFirstSearchHelper(Vertex v, LinkedList<Vertex> visited) {
        visited.add(v);
        for (int i = 0; i < v.edgesLeaving.size(); ++i) {
            Edge e = v.edgesLeaving.get(i);
            if (!visited.contains(e.target)) {

                depthFirstSearchHelper(e.target, visited);
            }
        }
    ...
    /**
     * DepthFirstSearch implemented for the print method. Will store all the
     * vertices in a list. Returns a list of the items.
     * 
     * @param item T - item to start at. Will be made into a vertex for helper
     *             method.
     * @return LinkedList<T> - LinkedList of all the elements in the graph using
     *         depth first search.
     */
    public LinkedList<T> depthFirstSearch(T item) {
        LinkedList<T> items = new LinkedList<>();
        LinkedList<Vertex> visited = new LinkedList<>();
        Vertex vertexNode = null;
        // Find and save desired vertex
        if (this.vertices.containsKey(item)) {
            vertexNode = this.vertices.get(item);
        } else {
            throw new IllegalArgumentException("Error! No vertex found!");
        }
        depthFirstSearchHelper(vertexNode, visited);
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
            ...
            for (int i = 0; i < source.dataSequence.size(); i++) {
                newList.add(source.dataSequence.get(i));
            }

            dataSequence = newList;
            start = source.start;
            end = source.end;
        }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        if (start == null || end == null || !vertices.containsKey(start) || !vertices.containsKey(end)) {
            throw new NoSuchElementException("No path to end vertex found.");
        }
        ...
        // create new priority queue to process paths
        PriorityQueue<Path> pathsToProcess = new PriorityQueue<>();
        pathsToProcess.add(shortestPath);

        while (!pathsToProcess.isEmpty()) {
            currentPath = pathsToProcess.remove(); // path with lowest weight
            Vertex currVertex = currentPath.end;
            visited.add(currVertex);

            if (currVertex.equals(vertices.get(end))) {
                break;
            }

            // process all unvisited targets/successors of current vertex
            for (int i = 0; i < currVertex.edgesLeaving.size(); ++i) {
                Edge currEdge = currVertex.edgesLeaving.get(i);
                Vertex currTarget = currEdge.target;

                Path newPath = new Path(currentPath);

                // check if current target has already been visited - do not process if true
                if (!visited.contains(currTarget)) {
                    newPath.extend(currEdge);
                    pathsToProcess.add(newPath);
                }
            }
        }

        if (currentPath == null || currentPath.dataSequence.contains(end)) {
            shortestPath = currentPath;
        } else {
            throw new NoSuchElementException("No path to end vertex found.");
        }

        return currentPath; // return the shortest path between two vertices in the graph
    }
